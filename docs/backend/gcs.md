...
- [Test Using an S3 Client](#test-using-s3-client)
...
docker run -p 9000:9000 --name gcs-s3 \
 ...
 ghcr.io/obstor/obstor backend gcs yourprojectid
```
...
## <a name="test-using-s3-client"></a>3. Test Using an S3 Client

You can interact with the server using the AWS CLI or rclone. Both provide commands for listing, copying, and mirroring objects, and support file systems and S3-compatible cloud storage services (AWS Signature v2 and v4).

### 3.1 Configure your client

Configure an rclone S3 remote once:

```bash
rclone config create obstor s3 provider=Other endpoint=http://backend-ip:9000 access_key_id=obstoraccesskey secret_access_key=obstorsecretkey
```
