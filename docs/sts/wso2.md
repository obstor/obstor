...
WSO2 generates tokens in first style by default, but if to be used with Obstor we should configure WSO2 to provide JWT tokens instead.
...
Using the above `access_token` we can perform an STS request to Obstor to get temporary credentials for Obstor API operations. Obstor STS API uses [JSON Web Key Set Endpoint](https://docs.wso2.com/display/IS541/JSON+Web+Key+Set+Endpoint) to validate if JWT is valid and is properly signed.
...
Assuming that Obstor server is configured to support STS API by following the doc [Obstor STS Quickstart Guide](/docs/sts), execute the following command to temporary credentials from Obstor server.
```bash
