...
func (g *Azure) NewBackendLayer(creds auth.Credentials) (obstor.ObjectLayer, error) {
	...
	azureUploadChunkSize, err = env.GetInt("OBSTOR_AZURE_CHUNK_SIZE_MB", azureDefaultUploadChunkSizeMB)
	if err != nil {
		return nil, err
	}
	azureUploadChunkSize *= humanize.MiByte
	if azureUploadChunkSize <= 0 || azureUploadChunkSize > 100*humanize.MiByte {
		return nil, fmt.Errorf("OBSTOR_AZURE_CHUNK_SIZE_MB should be an integer value between 0 and 100")
	}

	azureUploadConcurrency, err = env.GetInt("OBSTOR_AZURE_UPLOAD_CONCURRENCY", 4)
	if err != nil {
		return nil, err
	}
...
func (a *azureObjects) CopyObject(ctx context.Context, srcBucket, srcObject, destBucket, destObject string, srcInfo obstor.ObjectInfo, srcOpts, dstOpts obstor.ObjectOptions) (objInfo obstor.ObjectInfo, err error) {
	...
	res, err := destBlob.StartCopyFromURL(ctx, srcBlobURL, azureMeta, azblob.ModifiedAccessConditions{}, azblob.BlobAccessConditions{}, azblob.AccessTierNone, azblob.BlobTagsMap{})
	if err != nil {
		return objInfo, azureToObjectError(err, srcBucket, srcObject)
	}
	...
	if _, ok := srcInfo.UserDefined["x-amz-storage-class"]; ok {
		_, err = destBlob.SetTier(ctx, s3StorageClassToAzureTier(srcInfo.UserDefined["x-amz-storage-class"]),
			azblob.LeaseAccessConditions{}, azblob.RehydratePriorityStandard)
		if err != nil {
			return objInfo, azureToObjectError(err, srcBucket, srcObject)
		}
	}
...
func (a *azureObjects) NewMultipartUpload(ctx context.Context, bucket, object string, opts obstor.ObjectOptions) (uploadID string, err error) {
	...
	_, err = blobURL.Upload(ctx, bytes.NewReader(jsonData), azblob.BlobHTTPHeaders{}, azblob.Metadata{}, azblob.BlobAccessConditions{}, azblob.AccessTierNone, azblob.BlobTagsMap{}, azblob.ClientProvidedKeyOptions{}, azblob.ImmutabilityPolicyOptions{})
	if err != nil {
