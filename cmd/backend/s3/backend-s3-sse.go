...
/*
 ...
 NewMultipartUpload is stored in obj/.obstor/uploadID/dare.meta and each
 UploadPart operation saves additional state of the part's encrypted ETag and
 encrypted size in obj/.obstor/uploadID/part1/part.meta

 All the part metadata and temp dare.meta are cleaned up when upload completes
*/

// ListObjects lists all blobs in S3 bucket filtered by prefix
func (l *s3EncObjects) ListObjects(ctx context.Context, bucket string, prefix string, marker string, delimiter string, maxKeys int) (loi obstor.ListObjectsInfo, e error) {
...
func (l *s3EncObjects) ListObjectsV2(ctx context.Context, bucket, prefix, continuationToken, delimiter string, maxKeys int, fetchOwner bool, startAfter string) (loi obstor.ListObjectsV2Info, e error) {
	...
	for {
		...
		if e != nil {
			return loi, obstor.ErrorRespToObjectError(e, bucket)
		}
		...
		for _, p := range loi.Prefixes {
			objName := strings.TrimSuffix(p, obstor.SlashSeparator)
	...
	loi.Objects = make([]obstor.ObjectInfo, 0)
...
func isGWObject(objName string) bool {
	...
	pfxSlice := strings.Split(objName, obstor.SlashSeparator)
	var i1, i2 int
...
func (l *s3EncObjects) isGWEncrypted(ctx context.Context, bucket, object string) bool {
	_, err := l.s3Objects.GetObjectInfo(ctx, bucket, getDareMetaPath(object), obstor.ObjectOptions{})
	return err == nil
}
...
func (l *s3EncObjects) getGWMetadata(ctx context.Context, bucket, metaFileName string) (m gwMetaV1, err error) {
	oi, err1 := l.s3Objects.GetObjectInfo(ctx, bucket, metaFileName, obstor.ObjectOptions{})
	if err1 != nil {
		return m, err1
	}
	var buffer bytes.Buffer
	err = l.s3Objects.getObject(ctx, bucket, metaFileName, 0, oi.Size, &buffer, oi.ETag, obstor.ObjectOptions{})
	if err != nil {
		return m, err
	}
...
func (l *s3EncObjects) getObject(ctx context.Context, bucket string, key string, startOffset int64, length int64, writer io.Writer, etag string, opts obstor.ObjectOptions) error {
	...
	if startOffset > dmeta.Stat.Size || startOffset+length > dmeta.Stat.Size {
		logger.LogIf(ctx, obstor.InvalidRange{OffsetBegin: startOffset, OffsetEnd: length, ResourceSize: dmeta.Stat.Size})
		return obstor.InvalidRange{OffsetBegin: startOffset, OffsetEnd: length, ResourceSize: dmeta.Stat.Size}
	}
...
func (l *s3EncObjects) CopyObject(ctx context.Context, srcBucket string, srcObject string, dstBucket string, dstObject string, srcInfo obstor.ObjectInfo, s, d obstor.ObjectOptions) (objInfo obstor.ObjectInfo, err error) {
	...
	if cpSrcDstSame {
		...
		if s.ServerSideEncryption != nil && d.ServerSideEncryption != nil &&
			...
			if err = l.writeGWMetadata(ctx, dstBucket, getDareMetaPath(dstObject), gwMeta, obstor.ObjectOptions{}); err != nil {
				return objInfo, obstor.ErrorRespToObjectError(err)
			}
			return gwMeta.ToObjectInfo(dstBucket, dstObject), nil
		}
	}
	dstOpts := obstor.ObjectOptions{ServerSideEncryption: d.ServerSideEncryption, UserDefined: srcInfo.UserDefined}
...
func (l *s3EncObjects) DeleteObjects(ctx context.Context, bucket string, objects []obstor.ObjectToDelete, opts obstor.ObjectOptions) ([]obstor.DeletedObject, []error) {
...
func (l *s3EncObjects) ListMultipartUploads(ctx context.Context, bucket string, prefix string, keyMarker string, uploadIDMarker string, delimiter string, maxUploads int) (lmi obstor.ListMultipartsInfo, e error) {

	lmi, e = l.s3Objects.ListMultipartUploads(ctx, bucket, prefix, keyMarker, uploadIDMarker, delimiter, maxUploads)
	if e != nil {
		return
	}
	lmi.KeyMarker = strings.TrimSuffix(lmi.KeyMarker, getGWContentPath(obstor.SlashSeparator))
	lmi.NextKeyMarker = strings.TrimSuffix(lmi.NextKeyMarker, getGWContentPath(obstor.SlashSeparator))
	for i := range lmi.Uploads {
		lmi.Uploads[i].Object = strings.TrimSuffix(lmi.Uploads[i].Object, getGWContentPath(obstor.SlashSeparator))
	}
	return
}

// NewMultipartUpload uploads object in multiple parts
func (l *s3EncObjects) NewMultipartUpload(ctx context.Context, bucket string, object string, o obstor.ObjectOptions) (uploadID string, err error) {
	...
	uploadID, err = l.s3Objects.NewMultipartUpload(ctx, bucket, getGWContentPath(object), obstor.ObjectOptions{ServerSideEncryption: sseOpts})
	if err != nil {
		return
	}
	...
	err = l.writeGWMetadata(ctx, bucket, getTmpDareMetaPath(object, uploadID), gwmeta, obstor.ObjectOptions{})
	if err != nil {
		return uploadID, obstor.ErrorRespToObjectError(err)
	}
	return uploadID, nil
}

// PutObject creates a new object with the incoming data,
func (l *s3EncObjects) PutObject(ctx context.Context, bucket string, object string, data *obstor.PutObjReader, opts obstor.ObjectOptions) (objInfo obstor.ObjectInfo, err error) {
	var sseOpts encrypt.ServerSide
	// Decide if sse options needed to be passed to backend
	...
	oi, err := l.s3Objects.PutObject(ctx, bucket, getGWContentPath(object), data, obstor.ObjectOptions{ServerSideEncryption: sseOpts})
	if err != nil {
		return objInfo, obstor.ErrorRespToObjectError(err)
	}
	...
	gwMeta.Stat.ModTime = time.Now().UTC()
	if err = l.writeGWMetadata(ctx, bucket, getDareMetaPath(object), gwMeta, obstor.ObjectOptions{}); err != nil {
		return objInfo, obstor.ErrorRespToObjectError(err)
	}
...
func (l *s3EncObjects) PutObjectPart(ctx context.Context, bucket string, object string, uploadID string, partID int, data *obstor.PutObjReader, opts obstor.ObjectOptions) (pi obstor.PartInfo, e error) {
	...
	_, err := l.s3Objects.GetObjectInfo(ctx, bucket, tmpDareMeta, obstor.ObjectOptions{})
	if err != nil {
		return pi, obstor.InvalidUploadID{UploadID: uploadID}
	}
	...
	gwMeta := newGWMetaV1()
	gwMeta.Parts = make([]obstor.ObjectPartInfo, 1)
	// Add incoming part.
	gwMeta.Parts[0] = obstor.ObjectPartInfo{
		Number: partID,
		ETag:   pi.ETag,
		Size:   pi.Size,
	}
	...
	if err = l.writeGWMetadata(ctx, bucket, getPartMetaPath(object, uploadID, partID), gwMeta, obstor.ObjectOptions{}); err != nil {
		return pi, obstor.ErrorRespToObjectError(err)
	}
	return obstor.PartInfo{
		Size:         gwMeta.Stat.Size,
		ETag:         obstor.CanonicalizeETag(gwMeta.ETag),
...
func (l *s3EncObjects) GetMultipartInfo(ctx context.Context, bucket, object, uploadID string, opts obstor.ObjectOptions) (result obstor.MultipartInfo, err error) {
...
func (l *s3EncObjects) ListObjectParts(ctx context.Context, bucket string, object string, uploadID string, partNumberMarker int, maxParts int, opts obstor.ObjectOptions) (lpi obstor.ListPartsInfo, e error) {
	// We do not store parts uploaded so far in the dare.meta. Only CompleteMultipartUpload finalizes the parts under upload prefix.Otherwise,
	// there could be situations of dare.meta getting corrupted by competing upload parts.
	dm, err := l.getGWMetadata(ctx, bucket, getTmpDareMetaPath(object, uploadID))
	if err != nil {
...
func (l *s3EncObjects) AbortMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, opts obstor.ObjectOptions) error {
	...
	for {
		...
		for _, obj := range loi.Objects {
			if _, err := l.s3Objects.DeleteObject(ctx, bucket, obj.Name, obstor.ObjectOptions{}); err != nil {
				return obstor.ErrorRespToObjectError(err)
			}
			startAfter = obj.Name
		}
		continuationToken = loi.NextContinuationToken
...
func (l *s3EncObjects) CompleteMultipartUpload(ctx context.Context, bucket, object, uploadID string, uploadedParts []obstor.CompletePart, opts obstor.ObjectOptions) (oi obstor.ObjectInfo, e error) {
	...
	gwMeta.Parts = make([]obstor.ObjectPartInfo, len(uploadedParts))

	bkUploadedParts := make([]obstor.CompletePart, len(uploadedParts))
	...
	for i, part := range uploadedParts {
		partMeta, err := l.getGWMetadata(ctx, bucket, getPartMetaPath(object, uploadID, part.PartNumber))
		if err != nil || len(partMeta.Parts) == 0 {
			return oi, obstor.InvalidPart{}
		}
		bkUploadedParts[i] = obstor.CompletePart{PartNumber: part.PartNumber, ETag: partMeta.Parts[0].ETag}
	...
	if err = l.writeGWMetadata(ctx, bucket, getDareMetaPath(object), gwMeta, obstor.ObjectOptions{}); err != nil {
		return oi, obstor.ErrorRespToObjectError(err)
	}
