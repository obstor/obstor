	...
	for index, err := range dErrs {
		if err == errVolumeNotEmpty {
			...
			if derr := storageDisks[index].DeleteVol(ctx, bucket, false); derr == errVolumeNotEmpty {
				...
				_ = storageDisks[index].Delete(ctx, obstorMetaMultipartBucket, bucket, true)
			}
		}
	}
}
	...
	for index := range storageDisks {
		...
		g.Go(func() error {
			if storageDisks[index] != nil {
				...
				if err := storageDisks[index].Delete(ctx, obstorMetaMultipartBucket, bucket, true); err != errFileNotFound {
					return err
				}
				return nil
			}
