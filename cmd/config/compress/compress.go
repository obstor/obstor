...
const (
	...
	EnvCompressState           = "OBSTOR_COMPRESS_ENABLE"
	EnvCompressAllowEncryption = "OBSTOR_COMPRESS_ALLOW_ENCRYPTION"
	EnvCompressExtensions      = "OBSTOR_COMPRESS_EXTENSIONS"
	EnvCompressMimeTypes       = "OBSTOR_COMPRESS_MIME_TYPES"
...
func LookupConfig(kvs config.KVS) (Config, error) {
	...
	if compressExtensions != "" || compressMimeTypes != "" || compressMimeTypesLegacy != "" {
		if compressExtensions != "" {
			...
			if err != nil {
				return cfg, fmt.Errorf("%s: Invalid OBSTOR_COMPRESS_EXTENSIONS value (`%s`)", err, extensions)
			}
		...
		if compressMimeTypes != "" {
			...
			if err != nil {
				return cfg, fmt.Errorf("%s: Invalid OBSTOR_COMPRESS_MIME_TYPES value (`%s`)", err, mimeTypes)
			}
