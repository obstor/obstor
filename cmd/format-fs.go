...
func createFormatFS(fsFormatPath string) error {
	...
	if fi.Size() != 0 {
		// format.json already got created because of another obstor process's createFormatFS()
		return nil
	}
...
func initFormatFS(ctx context.Context, fsPath string) (rlk *lock.RLockedFile, err error) {
	...
	for {
		...
		if err == nil {
			...
			// obstor process just created the file but could not hold lock
			// and write to it.
			var fi os.FileInfo
			fi, err = rlk.Stat()
			if err != nil {
		...
		if osIsNotExist(err) || isEmpty {
			...
			if err == lock.ErrAlreadyLocked {
				...
				// Can happen in a rare situation when a parallel obstor process
...
func formatFSFixDeploymentID(ctx context.Context, fsFormatPath string) error {
	...
	for !stop {
		...
		default:
			...
			if err == lock.ErrAlreadyLocked {
				// Lock already present, sleep and attempt again
				logger.Info("Another obstor process(es) might be holding a lock to the file %s. Please kill that obstor process(es) (elapsed %s)\n", fsFormatPath, getElapsedTime())
