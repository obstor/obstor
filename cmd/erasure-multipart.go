...
func (er erasureObjects) checkUploadIDExists(ctx context.Context, bucket, object, uploadID string) (err error) {
	...
	metaArr, errs := readAllFileInfo(ctx, disks, obstorMetaMultipartBucket, er.getUploadIDDir(bucket, object, uploadID), "", false)
...
// Removes part given by partName belonging to a mulitpart upload from obstorMetaBucket
func (er erasureObjects) removeObjectPart(bucket, object, uploadID, dataDir string, partNumber int) {
	...
	for index, disk := range storageDisks {
		...
		g.Go(func() error {
			...
			_ = storageDisks[index].Delete(context.TODO(), obstorMetaMultipartBucket, curpartPath, false)
			return nil
		}, index)
	}
	g.Wait()
...
func (er erasureObjects) renameAll(ctx context.Context, bucket, prefix string) {
	...
	for _, disk := range er.getDisks() {
		...
		go func(disk StorageAPI) {
			defer wg.Done()
			_ = disk.RenameFile(ctx, bucket, prefix, obstorMetaTmpBucket, mustGetUUID())
		}(disk)
	}
	wg.Wait()
}
...
func (er erasureObjects) deleteAll(ctx context.Context, bucket, prefix string) {
	...
	for _, disk := range er.getDisks() {
		...
		go func(disk StorageAPI) {
			defer wg.Done()
			_ = disk.Delete(ctx, bucket, prefix, true)
		}(disk)
	}
	wg.Wait()
}
...
func (er erasureObjects) cleanupStaleUploadsOnDisk(ctx context.Context, disk StorageAPI, expiry time.Duration) {
	...
	readDirFn(pathJoin(diskPath, obstorMetaMultipartBucket), func(shaDir string, typ os.FileMode) error {
		return readDirFn(pathJoin(diskPath, obstorMetaMultipartBucket, shaDir), func(uploadIDDir string, typ os.FileMode) error {
			uploadIDPath := pathJoin(shaDir, uploadIDDir)
			fi, err := disk.ReadVersion(ctx, obstorMetaMultipartBucket, uploadIDPath, "", false)
			if err != nil {
				return nil
			}
			wait := er.deletedCleanupSleeper.Timer(ctx)
			if now.Sub(fi.ModTime) > expiry {
				er.renameAll(ctx, obstorMetaMultipartBucket, uploadIDPath)
			}
			wait()
			return nil
		})
	...
	readDirFn(pathJoin(diskPath, obstorMetaTmpBucket), func(tmpDir string, typ os.FileMode) error {
		if tmpDir == ".trash/" { // do not remove .trash/ here, it has its own routines
			return nil
		}
		vi, err := disk.StatVol(ctx, pathJoin(obstorMetaTmpBucket, tmpDir))
		if err != nil {
			return nil
		}
		wait := er.deletedCleanupSleeper.Timer(ctx)
		if now.Sub(vi.Created) > expiry {
			er.deleteAll(ctx, obstorMetaTmpBucket, tmpDir)
		}
		wait()
		return nil
	})
...
func (er erasureObjects) ListMultipartUploads(ctx context.Context, bucket, object, keyMarker, uploadIDMarker, delimiter string, maxUploads int) (result ListMultipartsInfo, err error) {
	...
	for _, disk = range er.getLoadBalancedDisks(true) {
		uploadIDs, err = disk.ListDir(ctx, obstorMetaMultipartBucket, er.getMultipartSHADir(bucket, object), -1)
		if err != nil {
	...
	for _, uploadID := range uploadIDs {
		...
		fi, err := disk.ReadVersion(ctx, obstorMetaMultipartBucket, pathJoin(er.getUploadIDDir(bucket, object, uploadID)), "", false)
		if err != nil {
			return result, toObjectErr(err, bucket, object)
		}
...
func (er erasureObjects) newMultipartUpload(ctx context.Context, bucket string, object string, opts ObjectOptions) (string, error) {
	...
	if _, err := writeUniqueFileInfo(ctx, onlineDisks, obstorMetaMultipartBucket, uploadIDPath, partsMetadata, writeQuorum); err != nil {
		return "", toObjectErr(err, obstorMetaMultipartBucket, uploadIDPath)
	}
...
func (er erasureObjects) PutObjectPart(ctx context.Context, bucket, object, uploadID string, partID int, r *PutObjReader, opts ObjectOptions) (pi PartInfo, err error) {
	...
	// Read metadata associated with the object from all disks.
	partsMetadata, errs = readAllFileInfo(ctx, storageDisks, obstorMetaMultipartBucket,
		uploadIDPath, "", false)
	...
	// Need a unique name for the part being written in obstorMetaBucket to
	...
	defer func() {
		if online != len(onlineDisks) {
			_ = er.deleteObject(context.Background(), obstorMetaTmpBucket, tmpPart, writeQuorum)
		}
	}()
	...
	for i, disk := range onlineDisks {
		...
		writers[i] = newBitrotWriter(disk, obstorMetaTmpBucket, tmpPartPath,
	...
	onlineDisks, err = rename(ctx, onlineDisks, obstorMetaTmpBucket, tmpPartPath, obstorMetaMultipartBucket, partPath, false, writeQuorum, nil)
	if err != nil {
		return pi, toObjectErr(err, obstorMetaMultipartBucket, partPath)
	}

	// Read metadata again because it might be updated with parallel upload of another part.
	partsMetadata, errs = readAllFileInfo(ctx, onlineDisks, obstorMetaMultipartBucket, uploadIDPath, "", false)
	reducedErr = reduceWriteQuorumErrs(ctx, errs, objectOpIgnoredErrs, writeQuorum)
	if reducedErr == errErasureWriteQuorum {
		return pi, toObjectErr(reducedErr, bucket, object)
	}
	...
	if _, err = writeUniqueFileInfo(ctx, onlineDisks, obstorMetaMultipartBucket, uploadIDPath, partsMetadata, writeQuorum); err != nil {
		return pi, toObjectErr(err, obstorMetaMultipartBucket, uploadIDPath)
	}
...
func (er erasureObjects) putBlockReplicatedPart(ctx context.Context, bucket, object, uploadID string,
	...
	*readLocked = false
	...
	// Re-read metadata.
	partsMetadata, errs := readAllFileInfo(ctx, storageDisks, obstorMetaMultipartBucket, uploadIDPath, "", false)
	reducedErr := reduceWriteQuorumErrs(ctx, errs, objectOpIgnoredErrs, writeQuorum)
	if reducedErr == errErasureWriteQuorum {
	...
	if _, err = writeUniqueFileInfo(ctx, onlineDisks, obstorMetaMultipartBucket, uploadIDPath, partsMetadata, writeQuorum); err != nil {
		return PartInfo{}, toObjectErr(err, obstorMetaMultipartBucket, uploadIDPath)
	}
...
func (er erasureObjects) GetMultipartInfo(ctx context.Context, bucket, object, uploadID string, opts ObjectOptions) (MultipartInfo, error) {
	...
	// Read metadata associated with the object from all disks.
	partsMetadata, errs := readAllFileInfo(ctx, storageDisks, obstorMetaMultipartBucket, uploadIDPath, opts.VersionID, false)

	// Get Quorum for this object
	readQuorum, _, err := objectQuorumFromMeta(ctx, partsMetadata, errs, er.defaultParityCount)
	if err != nil {
		return result, toObjectErr(err, obstorMetaMultipartBucket, uploadIDPath)
	}

	reducedErr := reduceWriteQuorumErrs(ctx, errs, objectOpIgnoredErrs, readQuorum)
	if reducedErr == errErasureReadQuorum {
		return result, toObjectErr(reducedErr, obstorMetaMultipartBucket, uploadIDPath)
	}
...
func (er erasureObjects) CompleteMultipartUpload(ctx context.Context, bucket string, object string, uploadID string, parts []CompletePart, opts ObjectOptions) (oi ObjectInfo, err error) {
	...
	if onlineDisks, err = writeUniqueFileInfo(ctx, onlineDisks, obstorMetaMultipartBucket, uploadIDPath, partsMetadata, writeQuorum); err != nil {
		return oi, toObjectErr(err, obstorMetaMultipartBucket, uploadIDPath)
	}
	...
	if onlineDisks, err = renameData(ctx, onlineDisks, obstorMetaMultipartBucket, uploadIDPath,
...
func (er erasureObjects) AbortMultipartUpload(ctx context.Context, bucket, object, uploadID string, opts ObjectOptions) (err error) {
	...
	// Read metadata associated with the object from all disks.
	partsMetadata, errs := readAllFileInfo(ctx, er.getDisks(), obstorMetaMultipartBucket, uploadIDPath, "", false)
	...
	if err = er.deleteObject(ctx, obstorMetaMultipartBucket, uploadIDPath, writeQuorum); err != nil {
		return toObjectErr(err, bucket, object, uploadID)
	}
